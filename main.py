...
=================================
串接四個階段：爬蟲 -> 正規化I -> OCR -> 正規化II（finalize）

只保留 final_output.csv 作為最終產出——這是給人看 / 之後給 LINE 推播模組用的
「交付物」，不是中間交接檔案，所以刻意保留。
"""
...
def run_pipeline():
    ...
    try:
        ...
        if image_urls:     #不匯出CSV檔資料僅保留在記憶體中
            ...
            all_results, all_dropped, all_errors, calls_used = [], [], [], 0

        # ---- [4/4] 正規化II：合併結果、寫回 Supabase ----
        print("\n" + "=" * 20 + " [4/4] 正規化II " + "=" * 20)
